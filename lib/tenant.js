...
import { sessionCaller } from './auth';
...
// A browser session cookie acts on the board that session belongs to, and additionally
// carries `user` — the only path that identifies a person rather than just a board.
export async function caller(req) {
  const token = bearer(req);
  if (token) {
    if (isAdminKey(token)) {
      const slug = req.headers.get('x-tenant') || new URL(req.url).searchParams.get('tenant');
      if (!slug) return { admin: true, tenant: null, user: null };
      const [tenant] = await sql`select * from tenants where slug=${slug}`;
      return tenant ? { admin: true, tenant, user: null } : null;
    }
    const [tenant] = await sql`select * from tenants where key_hash=${hashKey(token)}`;
    return tenant ? { admin: false, tenant, user: null } : null;
  }
  const s = await sessionCaller(req);
  return s ? { admin: false, tenant: s.tenant, user: s.user } : null;
}
...
export function withUser(handler, { adminOnly = false } = {}) {
  return async (req, ctx) => {
    ...
    if (!c) return json({ error: 'unauthorized' }, 401);
    if (!c.tenant) return json({ error: 'admin key needs an X-Tenant header' }, 400);
    if (!c.user && !c.admin) return json({ error: 'sign in to do that' }, 403);
    if (adminOnly && !isBoardAdmin(c)) return json({ error: 'admins only' }, 403);
    return handler(req, ctx, c.tenant, c);
  };
}

export const isBoardAdmin = (c) => c.admin === true || c.user?.is_admin === true;
