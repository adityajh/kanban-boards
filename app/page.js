...
const NAMES = ['Adi', 'Aadhya', 'Deepak'];
const TAGS = ['', 'A', 'B', 'Onboarding'];
...
export default function Board() {
  ...
  useEffect(() => {
    setKey(localStorage.getItem('board_key'));
    setUser(localStorage.getItem('board_user'));
  }, []);

  const api = useCallback(async (path, method = 'GET', body) => {
    const r = await fetch('/api' + path, {
      method,
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + (key || localStorage.getItem('board_key')) },
      body: body ? JSON.stringify(body) : undefined,
    });
    if (r.status === 401) { localStorage.removeItem('board_key'); setKey(null); throw new Error('Wrong passphrase'); }
  ...
  useEffect(() => { if (key && user) load(); }, [key, user, load]);

  // ---- gates ----
  if (!key) {
    return (
      <div className="gate">
        <div className="gatedots"><i/><i/><i/><i/><i/></div>
        <h1>INDITRESS</h1>
        ...
        <input type="password" value={pass} placeholder="Passphrase"
          ...
          onKeyDown={e => { if (e.key === 'Enter' && pass) { localStorage.setItem('board_key', pass); setKey(pass); setErr(''); } }} />
        <button className="btn" style={{ width: '100%' }}
          onClick={() => { if (pass) { localStorage.setItem('board_key', pass); setKey(pass); setErr(''); } }}>Enter</button>
      </div>
    );
  }
  if (!user) {
    return (
      <div className="gate">
        ...
        <div className="names">
          {NAMES.map(n => <button key={n} onClick={() => { localStorage.setItem('board_user', n); setUser(n); }}>{n}</button>)}
        </div>
      </div>
    );
  }
  ...
  const delCard = async (id) => { if (!confirm('Delete this project and all its subtasks, links and notes?')) return; await api('/cards/' + id, 'DELETE'); setCards(cs => cs.filter(c => c.id !== id)); setOpenId(null); };
  ...
  return (
    <>
      <div className="topbar">
        <div className="brand">INDITRESS<small>The Honest Hair Company</small></div>
        <div className="spacer" />
        <div className="dots" title="Origin · Cuticle · Processing · Performance · Supply"><i/><i/><i/><i/><i/></div>
        <div className="whoami">You are <b>{user}</b></div>
        <button className="linkbtn" onClick={() => { localStorage.removeItem('board_user'); setUser(null); }}>switch</button>
      </div>
                ...
                {list.map(c => {
                  ...
                  return (
                    <div key={c.id} className="card" draggable
                      ...
                      <div className="meta">
                        {c.tag && <span className={'tag ' + c.tag}>{c.tag}</span>}
      ...
      {open && <Detail card={open} user={user} api={api} reload={reload} patchCard={patchCard} delCard={delCard} close={() => setOpenId(null)} />}
    </>
  );
}

function Detail({ card, user, api, reload, patchCard, delCard, close }) {
      ...
      <div className="modal">
        ...
        <div className="row">
          <select value={c.assignee} onChange={e => saveField('assignee', e.target.value)}>
            {['Unassigned', ...NAMES].map(n => <option key={n}>{n}</option>)}
          </select>
          <select value={c.tag} onChange={e => saveField('tag', e.target.value)}>
            {TAGS.map(t => <option key={t} value={t}>{t ? 'Tag ' + t : 'No tag'}</option>)}
          </select>
